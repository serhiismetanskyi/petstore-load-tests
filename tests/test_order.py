"""Order load tests for PetStore API."""
    ...
    class OrderTasks(SequentialTaskSet):
        ...
        def order_create(self):
            ...
            with self.client.post(
                "/store/order", json=self.order_data, catch_response=True, name="Order Create"
            ) as response:
                task_result = ""
        ...
        def order_get_by_id(self):
            ...
            with self.client.get(
                f"/store/order/{self.order_id}", catch_response=True, name="Get Order by ID"
            ) as response:
                task_result = ""
        ...
        def order_delete(self):
            ...
            with self.client.delete(
                f"/store/order/{self.order_id}", catch_response=True, name="Order Delete"
            ) as response:
                task_result = ""
                ...
                if response.status_code == 200:
                    ...
                    task_result = f"{self.order_id} delete failed: Invalid ID supplied"
                    response.failure(task_result)
                elif response.status_code == 404:
                    task_result = f"{self.order_id} delete failed: Order not found"
                    response.failure(task_result)
                else:
