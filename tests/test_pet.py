"""Pet load tests for PetStore API."""
...
class UserBehavior(TaskSet):
    ...
    class PetTasks(SequentialTaskSet):
        ...
        def pet_update_img(self):
            ...
            with open("./data/pet_img.jpg", "rb") as img_file:
                files = {"file": img_file}
                with self.client.post(
                    f"/pet/{self.pet_id}/uploadImage",
                    data=data,
                    files=files,
                    catch_response=True,
                    name="Pet Image Update",
                ) as response:
                    task_result = ""
                    Logger.add_request(
                        test_name="Pet Image Update", url=f"/pet/{self.pet_id}/uploadImage", method="POST"
                    )
                    if response.status_code == 200:
        ...
        def pet_update_by_id(self):
            """Pet Update by ID"""
            with self.client.post(
                f"/pet/{self.pet_id}", data=self.pet_data_for_update, catch_response=True, name="Pet Update by ID"
            ) as response:
                task_result = ""
                Logger.add_request(
                    test_name="Pet Update by ID",
                    url=f"/pet/{self.pet_id}",
                    method="POST",
                    body=self.pet_data_for_update,
                )
                if response.status_code == 200:
        ...
        def pet_delete(self):
            ...
            with self.client.delete(
                f"/pet/{self.pet_id}", headers=headers, catch_response=True, name="Pet Delete"
            ) as response:
                task_result = ""
