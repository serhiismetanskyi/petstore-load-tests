"""User load tests for PetStore API."""
    ...
    class UserTasks(SequentialTaskSet):
        ...
        def user_registration(self):
            ...
            with self.client.post(
                "/user", json=self.user_data, catch_response=True, name="User Registration"
            ) as response:
                task_result = ""
        ...
        def user_login(self):
            ...
            with self.client.get(
                f"/user/login?username={self.user_name}&password={self.password}",
                catch_response=True,
                name="User Login",
            ) as response:
                task_result = ""
                Logger.add_request(
                    ...
                    url=f"/user/login?username={self.user_name}&password={self.password}",
                    method="GET",
                )
                if response.status_code == 200:
        ...
        def user_update(self):
            ...
            with self.client.put(
                f"/user/{self.user_name}", json=self.user_data_for_update, catch_response=True, name="User Update"
            ) as response:
                task_result = ""
                Logger.add_request(
                    test_name="User Update", url=f"/user/{self.user_name}", method="PUT", body=self.user_data_for_update
                )
                if response.status_code == 200:
        ...
        def user_delete(self):
            ...
            with self.client.delete(f"/user/{self.user_name}", catch_response=True, name="User Delete") as response:
                ...
                if response.status_code == 200:
                    ...
                    task_result = f"{self.user_name} delete failed: User not found"
                    response.failure(task_result)
                else:
                    task_result = f"{self.user_name} delete failed: Status code is {response.status_code}"
                    response.failure(task_result)
                Logger.add_response(response, task_result)
